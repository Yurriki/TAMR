...
from visualizer import VisualizerDsp
...
def main():
    ...
    for ii, data in tqdm(enumerate(data_loader)):
        ...
        with torch.no_grad():
            ...
            vis.draw_once(output, data, show_map=True, test_mode=False)

            results = [x[0:1].detach().cpu().numpy() for x in output["reg"]]
            results_cls = [softmax(x[0:1]).detach().cpu().numpy() for x in output["cls"]]
        for i, (argo_idx, pred_traj,pred_cls) in enumerate(zip(data["argo_id"], results,results_cls)):
            preds[argo_idx] = pred_traj.squeeze()
            cls[argo_idx] = pred_cls.squeeze()
    ...
    if args.split == "val":
        ...
        generate_forecasting_h5(preds, f"{config['save_dir']}/submitwithproba.h5",probabilities=cls)  # this might take awhile
