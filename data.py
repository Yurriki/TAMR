...
class ArgoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                new_data = dict()
                for key in ['city', 'orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'ctrs', 'graph','trajs','steps','used_trajs']:
                    if key in data:
                        new_data[key] = ref_copy(data[key])
                data = new_data
    ...
    def get_obj_feats(self, data):
        ...
        feats, ctrs, gt_preds, has_preds,used_trajs = [], [], [], [],[]
        for traj, step in zip(data['trajs'], data['steps']):
            if 19 not in step:
                continue

            gt_pred = np.zeros((30, 2), np.float32)
            has_pred = np.zeros(30, np.bool_)
            ...
            used_traj = np.zeros((20, 2), np.float32)
            used_traj[step, :2] = traj
            ...
            feat[step[0], :2] = 0
            feats.append(feat)
            used_trajs.append(used_traj)
            gt_preds.append(gt_pred)
            has_preds.append(has_pred)

        feats = np.asarray(feats, np.float32)
        used_trajs = np.asarray(used_trajs, np.float32)
        ctrs = np.asarray(ctrs, np.float32)
        gt_preds = np.asarray(gt_preds, np.float32)
        has_preds = np.asarray(has_preds, np.bool_)

        data['feats'] = feats
        data['used_trajs'] = used_trajs
        data['ctrs'] = ctrs
        data['orig'] = orig
        data['theta'] = theta
        data['rot'] = rot
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                new_data = dict()
                for key in ['orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'ctrs', 'graph','argo_id','city','trajs','steps','used_trajs']:
                    if key in data:
                        new_data[key] = ref_copy(data[key])
                data = new_data
            return data
