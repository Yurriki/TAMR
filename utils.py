...
def load_pretrain_test(net, pretrain_dict):
    custom_mappings = {
    "actor_net1": "actor_net20",
    "reconstruction": "reconstruction20"
    }
    state_dict = net.state_dict()
    new_pretrain_dict = {}

    for key, value in pretrain_dict.items():
        mapped_key = key  # 默认键名保持不变
        
        # 遍历映射规则，进行匹配和替换
        for prefix, new_prefix in custom_mappings.items():
            if key.startswith(prefix):
                mapped_key = key.replace(prefix, new_prefix, 1)
                break
        
        # 如果映射后的键在网络中，并且权重大小匹配，加入新的字典
        if mapped_key in state_dict and value.size() == state_dict[mapped_key].size():
            new_pretrain_dict[mapped_key] = value

    # 更新网络权重
    state_dict.update(new_pretrain_dict)
    net.load_state_dict(state_dict)

def load_pretrain_partial_freeze(net, pretrain_dict):
    state_dict = net.state_dict()
    for key in pretrain_dict.keys():
        if key in state_dict and (pretrain_dict[key].size() == state_dict[key].size()):
            value = pretrain_dict[key]
